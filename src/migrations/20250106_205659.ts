...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE UNIQUE INDEX \`gallery_filename_idx\` ON \`gallery\` (\`filename\`);`)
  ...
  await db.run(sql`CREATE UNIQUE INDEX \`services_filename_idx\` ON \`services\` (\`filename\`);`)
